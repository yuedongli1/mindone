...
from mindone.models.modules.utils import get_sequence_parallel_world_size, get_sequence_parallel_group, \
    get_sequence_parallel_rank, get_sequence_parallel_send_stream, get_sequence_parallel_recv_stream
...
class RingAttention(nn.Cell):
    ...
    def __init__(self,
        ...
        self.stream_send = get_sequence_parallel_send_stream()
        self.stream_recv = get_sequence_parallel_recv_stream()
    ...
    def forward_update(self, prev_attn_out, prev_softmax_max, prev_softmax_sum,
        ...
        if self.input_layout == "BSH":
            b1, _, s1, _ = prev_out_scale.shape
            prev_out_scale = ops.transpose(prev_out_scale, (0, 2, 1, 3))
            prev_out_scale = ops.reshape(prev_out_scale, (b1, s1, -1))
            # prev_out_scale = rearrange(prev_out_scale.asnumpy(), 'b n s d -> b s (n d)')
        ...
        if self.input_layout == "BSH":
            b1, _, s1, _ = cur_out_scale.shape
            cur_out_scale = ops.transpose(cur_out_scale, (0, 2, 1, 3))
            cur_out_scale = ops.reshape(cur_out_scale, (b1, s1, -1))
            # cur_out_scale = rearrange(cur_out_scale.asnumpy(), 'b n s d -> b s (n d)')
        elif self.input_layout == "SBH":
    ...
    def check_parameter(self, q, k, v, attn_mask, alibi_mask, prefix, padding_mask, attn_mask_type):
        ...
        if attn_mask_type is not 'causal' and attn_mask_type is not 'full':
    ...
    def construct(self, q, k, v, attn_mask=None, alibi_mask=None, prefix=None, padding_mask=None,
        ...
        # split chunk[i]~chunk[cp_size-i-1] into chunk[i] and chunk[cp_size-i-1],, [b, 2s, h] -> [b, 2, s, h]
        if causal:
        ...
        send_kv = ops.cat((k.unsqueeze(0), v.unsqueeze(0)), axis=0)  # [2, b, 2, s, h]
        recv_tensor = None
        send_recv_ops = []
        ...
        for i in range(cp_size):
            ...
            if i == 0:
                ...
                cur_k, cur_v = send_kv[0], send_kv[1]  # [b, 2, s, h]
            ...
            # if i <= rank: [b, 2s, h], [b, n, 2s, 8], [b, n, 2s, 8]
            # else: [b, s, h], [b, n, s, 8], [b, n, s, 8]
